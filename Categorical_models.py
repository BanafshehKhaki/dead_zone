    ...
    for model_name in models:
        ...
        for target in targets:
            if target.find('category') > 0:
                cat = 1
                directory = 'Results/balance_data/output_Cat_' + model_name+'/oversampling_cv_models/'
                ...
                cat =0 
                directory = 'Results/balance_data/output_Reg_' + model_name+'/oversampling_cv_models/'
                data = {'target_names': 'target_names', 'method_names': 'method_names', 'window_nuggets': 'window_nuggets', 'temporalhorizons': 'temporalhorizons', 'CV': 'CV',
                    'file_names': 'file_names',  'std_test_score': 'std_test_score', 'mean_test_score': 'mean_test_score', 'params': 'params', 'bestscore': 'bestscore','mape':'mape', 'me':'me', 'mae':'mae','mpe':'mpe','rmse':'rmse','corr':'corr', 'R2':'R2', 'imfeatures': 'imfeatures'}
            ...
            for n_steps in [1, 3, 6, 12]:
                for PrH_index in [1, 3, 6, 12, 24,36,48]:
                    ...
                    for train_index, test_index in custom_cv:
                        ...
                        # predict_mine = []
                        fpath = 'predictions_' + method+target+'_Window'+str(n_steps) +'_TH'+str(PrH_index)+'_CV' + str(i)+file
                        ...
                        if cat ==1:
                            ...
                            best_thresh = dict()
                            for i in range(3):
                                fpr[i], tpr[i], thresholds = roc_curve(y[:, i], yhat[:, i]) 
                                roc_auc[i] = auc(fpr[i], tpr[i])
                                J = tpr[i] - fpr[i]
                                # get the best threshold
                                ix = argmax(J)
                                best_thresh[i] = thresholds[ix]
                                print('Best Threshold=%f, roc_auc=%.3f' % (best_thresh[i], roc_auc[i]))
                        ...
                        if cat ==0:
                            predictions, test_y = func.transform(predictions, test_y, method, target, file)
                        ...
                        if cat ==1:
                            ...
                            data = {'target_names': target, 'method_names': method, 'window_nuggets': n_steps, 'temporalhorizons': PrH_index, 'CV': i,
                                'file_names': fpath, 'std_test_score': [test_std], 'mean_test_score': [test_Score], 'params': [clf.best_params_], 'bestscore': [clf.best_score_], 'mape':cm0[0], 'me':cm0[1], 'mae':cm0[2],'mpe':cm0[3],'rmse':cm0[4],'corr':cm0[5], 'R2':cm0[6], 'imfeatures': [clf.best_estimator_]}
