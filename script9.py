...
#####################################################################
def prediction_horizon(data, pd_steps):
...
#####################################################################	
def main(filePath, PrH_Steps, td_steps, PrH_name):
	fileName = filePath.split("/")
	df = pd.read_csv(filePath)
	
	target_values = prediction_horizon(df, PrH_Steps)
	
	df = df.drop(df.index[len(df.index)-PrH_Steps : len(df.index)], axis=0)
	
	new_data = df.drop(columns=['pHcategory','DOcategory','Target'])
	...
	new_datapd['pHcategory'] = df['pHcategory'] 
	new_datapd['DOcategory'] = df['DOcategory'] 
	new_datapd['Target'] = target_values
	
	# Saving in whiten/PrH_TD folder
	filePath = re.sub('whiten/','whiten/PrH_TD/',filePath)
...
if __name__ == "__main__":
	if len(sys.argv) ==5:
		...
		main(filePath, PrH_Steps,td_steps,PrH_name)
	else:
