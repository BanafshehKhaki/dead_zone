...
def main():
    ...
    targets =   ['ph']#,'dissolved_oxygen']#['DOcategory', 'pHcategory']
    ...
    for model_name in models:
        ...
        for target in targets:
            if target.find('category') > 0:
                ...
                data = {'target_names': 'target_names', 'method_names': 'method_names', 'temporalhorizons': 'temporalhorizons', 'CV': 'CV',
                    'file_names': 'file_names',  'mape':'mape', 'me':'me', 'mae':'mae','mpe':'mpe','rmse':'rmse','corr':'corr', 'R2':'R2'}
            ...
            for PrH_index in [1, 3, 6, 12, 24]:
                ...
                if cat ==1  and (model_name == 'LSTM' or model_name == 'NN'):
                    ...
                    clf = clf.fit(train_X_grid, train_y_grid,model__class_weight={0: 1, 1: 50, 2: 100})
                else:
                ...
                files = [f for f in os.listdir(testpath) if f.endswith('.csv')and f.startswith(testsondefilename)]
                file1 = files[0]
                print('Window: '+str(lags) + ' TH: ' + str(PrH_index)+' '+method+' '+target+file1)
                ...
                for test_index in custom_cv:
                    ...
                    if i <=3:
                        ...
                        plt.legend(['actual', 'predictions'],
                                   loc='upper right')
                        fpath = 'predictions_' + method+target+'_Window'+str(lags) +'_TH'+str(PrH_index)+'_CV' + str(i)+file1
                    ...
                    if cat ==1:
                        ...
                        data = {'target_names': target, 'method_names': method, 'temporalhorizons': PrH_index, 'CV': i,
                            'file_names': filename, 'mape':cm0[0], 'me':cm0[1], 'mae':cm0[2],'mpe':cm0[3],'rmse':cm0[4],'corr':cm0[5], 'R2':cm0[6]}
